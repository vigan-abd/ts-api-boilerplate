...
import { event as fireEvent } from '@helpers/EventHelper';
...
class AuthenticationAPIController {
  ...
  async signin(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      fireEvent(new UserLoggedInEvent(auth.userId));
      res.statusCode = 200;
      return res.json(auth);
    } catch (err) {
      return next(err);
   ...
   * 
   */
  currentUser(req: IAuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      res.statusCode = 200;
      return res.json((req.currentUser as User).asDTO());
    } catch (err) {
      return next(err);
    }
  }
   ...
   * 
   */
  async updatePassword(req: IAuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const auth = await this.userService.updatePassword(req);
      fireEvent(new UserUpdatedEvent(auth.userId));
      res.statusCode = 200;
      return res.json(auth);
    } catch (err) {
      return next(err);
